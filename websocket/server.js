...
const clients = new Map(); // userId -> { ws, userId, chatSubscriptions }
...
class WebSocketServer {
        ...
        const pingInterval = setInterval(() => {
          ...
          if (client) {
            ...
            if (now - client.lastPing > 30000) {
              console.log(`User ${userId} ping timeout`);
              ws.terminate();
  ...
  async handleMessage(userId, ws, data) {
    ...
    switch (data.type) {
      case 'subscribe':
        client.chatSubscriptions.add(data.chatId);
        ws.send(JSON.stringify({
          type: 'subscribed',
          chatId: data.chatId
        }));
      ...
      case 'unsubscribe':
        client.chatSubscriptions.delete(data.chatId);
        ws.send(JSON.stringify({
          type: 'unsubscribed',
          chatId: data.chatId
        }));
  ...
  broadcastToChat(chatId, message, excludeUserId = null) {
    clients.forEach((client, userId) => {
      ...
      if (client.chatSubscriptions.has(chatId) && client.ws.readyState === WebSocket.OPEN) {
